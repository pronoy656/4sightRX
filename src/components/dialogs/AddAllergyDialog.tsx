...
export function AddAllergyDialog({ open, onOpenChange, onSuccess }: AddAllergyDialogProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axiosSecure.post("/allergies", {
                name: formData.name,
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden border-none shadow-2xl rounded-2xl bg-white focus-visible:outline-none">
                <DialogHeader className="p-6 border-b border-slate-100 flex flex-row items-center justify-between space-y-0 sticky top-0 bg-white z-10">
                    <DialogTitle className="text-xl font-bold text-slate-800">Add New Allergy</DialogTitle>
                ...
                <div className="p-8 pb-10 space-y-5">
                    ...
                    <div className="flex items-center gap-4 pt-6">
                        ...
                        <Button
                            ...
                            disabled={loading}
                        >
                            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Add Allergy"}
                        </Button>
                    </div>
                </div>
            </DialogContent>
